...
class DStarLite {
 public:
  ...
  /**
   * @brief Initialisation function of D*. Initialises G and RHS values for all
   * nodes, store motion allowable primatives, km value and the first value of
   * the priority queue.
   * @return void
   */
  void Init();
  ...
  /**
   * @brief Compare keys function for D* Lite. Compares the key given as input
   * with the key values of the node given as input. Calls CalculateKey on the
   * node.
   ...
   */
  std::vector<Node> d_star_lite(std::vector<std::vector<int>>& grid_in,
  ...
  * @brief Replan route, called whenever a previously unknown obstacle is
  ...
  detectedd in the while loop within the main procedure of D* Lite.
  ...
  */
  ...
  /**
   * @brief As D* Lite moves from goal to start, inverts the path vector taht
   * has been generated as well as the cost, ensuring that cost to start is 0.
   * @return path vector of nodes
   */
  std::vector<Node> ReturnInvertedVector() const;

  /**
   * @brief Function to run D* Lite live, showing the movement of the bot with
   * time. Timeout after each movement set in .h file. Next point in path might
   * beset to obstacle with probability 1/n. Calls UpdateStart and SetObs.
 ...
 private:
  bool disp_inc =
      true;  // Display incremental movements during D* Lite live run
  std::chrono::milliseconds disp_p{
      500};  // Pause for displaying grid in microseconds
  Node start_, main_start_, goal_, last_;
  std::vector<std::vector<std::pair<double, double>>>
      S_;  // Do not let grid size exceed 100

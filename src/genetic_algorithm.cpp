      ...
      if (f_vals[i] <= f_val * static_cast<int>(c_)) {
        if (shorten_chromosome_) {
          new_paths_.emplace_back(std::vector<Node>(
              paths_[i].begin(),
              paths_[i].begin() + path_length_));  // c provides a margin of
...
std::vector<Node> GeneticAlgorithm::ReturnLastPath()
    const {  // given the way a genetic algorithm decreases fitness values, the
             // last path is likely ot be the best. Can reorder ased on actual
             // fitness values if required.
  std::vector<Node> v;
  v.push_back(start_);
  Node current = start_;
...
int GeneticAlgorithm::CalculateFitness(const std::vector<Node>& path) const {
  ...
  for (const auto& p : path) {
    ...
    if (grid_[i.x_][i.y_] == 1) {
      ...
      cost += abs(goal_.x_ - i.x_) +
              abs(goal_.y_ - i.y_);  // Can add a scaling factor here
    }
  }
  return cost;
}
...
void GeneticAlgorithm::CrossoverMutation() {
  ...
  Node current = start_;
  const int len =
      std::min(static_cast<int>(std::max(paths_[p1].size(), paths_[p2].size())),
               path_length_);
  for (int i = 0; i < len; i++) {
    ...
    // obstacles are still allowed
    if (current.x_ < 0 || current.x_ >= n_ || current.y_ < 0 ||
        current.y_ >= n_) {
      child.pop_back();
      current = tmp;
      i--;
...
bool GeneticAlgorithm::CheckPath(const std::vector<Node>& path) const {
  ...
  for (const auto& node : path) {
    ...
    if (current.x_ < 0 || current.x_ >= n_ || current.y_ < 0 ||
        current.y_ >= n_ || grid_[current.x_][current.y_] == 1) {
      return false;
    }
  }

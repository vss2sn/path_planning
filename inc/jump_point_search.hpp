...
public:
  std::vector<Node> jump_point_search(std::vector<std::vector<int>> &grid, const Node& start_in, const Node& goal_in);
  void InsertionSort(std::vector<Node>& v) const;
  bool has_forced_neighbours(const Node& new_point, const Node& next_point, const Node& motion) const ;
